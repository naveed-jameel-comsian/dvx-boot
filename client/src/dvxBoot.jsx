...
const DvxBot = () => {
  ...
  const [isMaintenanceServiceCarSelect, setMaintenanceServiceCarSelect] =
    useState(false);
  ...
  const [isAskNumber, setShowAskNumber] = useState(false);
  const [isAskEmail, setShowAskEmail] = useState(false);
  const [askNumber, setAskNumber] = useState(null);
  const [askEmail, setAskEmail] = useState(null);
  ...
  useEffect(() => {
    const lang = navigator.language.startsWith("fr")
      ? "fr"
          ...
          if (
            ...
            if (text.includes("main-btn3")) {
              ...
              botMessage = {
                ...
                key: "common-tips",
                html: true,
                buttons: ["price", "duration", "contact"],
              };
            } else {
              setSelectedOption(text);
              botMessage = {
        ...
        case "carSelection":
          if (carSelectionStep === "brand") {
            ...
            botMessage = {
              ...
              text: `${t("awsome")} ${carDetails.brand} ${carDetails.model} ${
                carDetails.year
              } ${carDetails.engine}. ${t("correct")}`,
              html: true,
              buttons: ["✅yes", "❌no"],
            };
        ...
        case "confirmCarSelection":
          if (text === "✅yes") {
            if (section == "3A") {
              botMessage = {
                type: "bot",
                text: `${t("tuining-data")} ${carDetails.brand} ${
                  carDetails.model
                } ${carDetails.year} ${carDetails.engine} 🚗⚡:`,
                html: true,
              ...
              botMessage = {
                ...
                key: "3b-option-text",
                html: true,
                buttons: ["3b-btn1", "3b-btn2", "3b-btn3"],
              };
        ...
        case "tuningOptions":
          if (text === "stage1") {
            ...
            if (text === "stage1") {
              let botMessage2 = {
                type: "bot",
                text: `${t("stage1-for")} ${carDetails.brand} ${
                  carDetails.model
                } ${carDetails.year} ${carDetails.engine}`,
                html: true,
              };
              ...
              let botMessage2 = {
                type: "bot",
                text: `${t("stage2-for")} ${carDetails.brand} ${
                  carDetails.model
                } ${carDetails.year} ${carDetails.engine}`,
                html: true,
              };
              ...
              let botMessage2 = {
                type: "bot",
                text: `${t("stage3-for")} ${carDetails.brand} ${
                  carDetails.model
                } ${carDetails.year} ${carDetails.engine}`,
                html: true,
              };
          ...
          } else if (text === "stage2" || text === "stage3") {
            setTuningStage(text);
            botMessage = {
              type: "bot",
              key: "cust-detail-msg",
              html: true,
              buttons: ["req-call", "req-email", "not-ready"],
            };
          } else if (text.toLowerCase() === "req-call") {
            botMessage = {
              type: "bot",
              key: "ask-number",
              html: true,
          ...
          } else if (text.toLowerCase() === "req-email") {
            botMessage = {
              type: "bot",
              key: "ask-email",
              html: true,
  ...
  const handleEnterLicense = async () => {
    ...
    try {
      ...
      if (res?.data?.isFound) {
        let recordFound = {
          type: "bot",
          text: `${t("history")} ${res?.data?.brand}, ${res?.data?.model}. ${t(
            "today-looking"
          )}`,
          html: true,
        };
  ...
  const handleBookAppointment = async () => {
    if (!dateTime || !time || time.length !== 2) {
        console.error("Please select a valid date and time range.");
        return;
    }
    ...
    const calenderTitle = 
    `
        ${maintenanceService ? t(maintenanceService) : t("service_title_chip")}- 
        ${firstName+" "+lastName}-
        ...
        ${time[0]+"-"+time[1]}
    `
    console.log("calenderTitle------------",calenderTitle)
    const [startTime, endTime] = time;
    const startDateTime = moment(dateTime)
        .set({ hour: moment(startTime, "HH:mm").hour(), minute: moment(startTime, "HH:mm").minute() })
        .format("YYYY-MM-DDTHH:mm:ssZ");

    const endDateTime = moment(dateTime)
        .set({ hour: moment(endTime, "HH:mm").hour(), minute: moment(endTime, "HH:mm").minute() })
        .format("YYYY-MM-DDTHH:mm:ssZ")
  ...
  const handleEnterAskNumber = () => {
    const userMessage = { type: "user", text: askNumber };
    setMessages((prevMessages) => [...prevMessages, userMessage]);
    setAskNumber(null);

    setIsTyping(true);
    setTimeout(() => {
      const botMessage = { type: "bot", key: "ask-thank" };
      setMessages((prevMessages) => [...prevMessages, botMessage]);
      setIsTyping(false);
  ...
  const handleEnterAskEmail = () => {
    const userMessage = { type: "user", text: askEmail };
    setMessages((prevMessages) => [...prevMessages, userMessage]);
    setAskEmail(null);
  ...
  const fetchBusySlots = async (dateString) => {
    if (!dateString) return;
    try {
      const response = await axios.post(`${baseUrl}/get-busy-slots`, {
        date: dateString,
      });
  ...
  const isDisabledTime = (time) => {
    return busySlots.some((slot) => {
      ...
      return moment(time, "HH:mm").isBetween(slotStart, slotEnd, null, "[)"); // Inclusive of start, exclusive of end
    });
  };
    ...
    <div className={`chatbot ${expanded ? "expanded" : ""}`}>
      <div className="header">
        <div className="profile">
          <img src="/logo.jpeg" alt="Avatar" className="avatar-image" />
        ...
        {messages.map((message, index) => {
          ...
          return (
            <div key={index} className={`message ${message.type}`}>
              {message.html
                ? parse(message.key ? t(message.key) : message.text)
                : message.key
                ? t(message.key)
                : message.text}
              {message.buttons && (
          ...
          <div className="book-section">
            ...
            <DatePicker
              ...
              onChange={(_, dateString) => {
                ...
                fetchBusySlots(dateString); // Fetch busy slots when date changes
              }}
              ...
              disabledTime={() => ({
                disabledHours: () => {
                  return Array.from({ length: 24 }, (_, hour) => hour).filter(
                    (hour) =>
                      busySlots.some((slot) => {
                        const slotStart = moment(slot.start, "HH:mm").hour();
                        const slotEnd = moment(slot.end, "HH:mm").hour();
                        return hour >= slotStart && hour < slotEnd; // Disable full hour range
                      })
                  );
                },
                disabledMinutes: (selectedHour) => {
                  return Array.from({ length: 60 }, (_, min) => min).filter(
                    (min) =>
                      busySlots.some((slot) => {
                        const slotStart = moment(slot.start, "HH:mm");
                        const slotEnd = moment(slot.end, "HH:mm");
        ...
        <div className="chat-input">
          <div className="input-row">
            <Input
              ...
              onPressEnter={() => handleEnterAskNumber()}
            />
            <SendOutlined
              className="send-icon"
              onClick={() => handleEnterAskNumber()}
            />
          </div>
        </div>
      )}
